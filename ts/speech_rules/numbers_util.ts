 ...
 */
 export function decreasedNumeralsConversion(node: Element): string {
  let children = XpathUtil.evalXPath('children/*', node) as Element[];
  return (children.length % 10 === 0 ? LOCALE.NUMBERS.tens[9+children.length/10] : LOCALE.NUMBERS.tens[10+children.length/10]) + LOCALE.NUMBERS.ones[9 + children.length % 10];
}