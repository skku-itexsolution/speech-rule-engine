 ...
 */
 export function numeralsConversion(node: Element): string {
  ...
  return children.length < 100 && LOCALE.NUMBERS.tens[10 + children.length / 10] + LOCALE.NUMBERS.ones[10 + children.length % 10];
}
 ...
 */
 export function decreasedNumeralsConversion(node: Element): string {
  let children = XpathUtil.evalXPath('children/*', node) as Element[];
  return children.length < 100 &&  (children.length % 10 === 0 ? 
    LOCALE.NUMBERS.tens[9+children.length/10] : LOCALE.NUMBERS.tens[10+children.length/10]) 
